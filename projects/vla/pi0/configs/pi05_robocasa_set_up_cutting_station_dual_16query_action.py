...
config = dict(
    ...
    project_dir='./experiments/vla/pi05/robocasa_set_up_cutting_station_dual_16query_action',
    launch=dict(
        gpu_ids=[4, 5, 6, 7],
    ...
    models=dict(
        # Match the validated dual baseline: load the original Pi0.5 Action
        # Expert, clone it into arm/base, and initialize only the query banks.
        ...
        query_init_std=0.02,
        action_expert_initialization='pi05',
        action_expert_random_seed=6666,
        reset_action_experts_after_load=False,
    ),
    optimizers=dict(
        type='AdamW',
        betas=(0.9, 0.95),
    ...
    train=dict(
        ...
        wandb=dict(
            ...
            mode='online',
            project='robocasa_pi',
            name='pi05_dual_16query_action_from_pi05',
            group='robocasa_set_up_cutting_station',
            job_type='train',
            tags=['robocasa', 'pi05', 'dual-action', '16-query', 'query-action-feedback', 'pi05-init'],
            config=dict(
                model_variant='pi05_dual_16query_action',
