...
config = dict(
    runners=['pi0.pi05_dual_query_action_trainer.Pi05DualQueryActionTrainer'],
    project_dir='./experiments/vla/pi05/robocasa_set_up_cutting_station_dual_16query_action_random',
    launch=dict(
        gpu_ids=[0, 1, 2, 3],
    ...
    models=dict(
        # Load the original Pi0.5 VLM, then deliberately reset both complete
        # action paths (arm/base towers, projections, AdaRMS and time MLP).
        ...
        action_expert_random_seed=6666,
        reset_action_experts_after_load=True,
    ),
    optimizers=dict(
        type='AdamW',
        betas=(0.9, 0.95),
    ...
    train=dict(
        ...
        wandb=dict(
            ...
            mode='online',
            project='robocasa_pi',
            name='pi05_dual_16query_action_random_heads',
            group='robocasa_set_up_cutting_station',
            job_type='train',
            tags=['robocasa', 'pi05', 'dual-action', '16-query', 'query-action-feedback', 'random-heads'],
            config=dict(
                model_variant='pi05_dual_16query_action_random',
                initialization='pi05_vlm_random_action_heads',
                action_expert_random_seed=6666,
                ...
                query_reads_matching_action=True,
