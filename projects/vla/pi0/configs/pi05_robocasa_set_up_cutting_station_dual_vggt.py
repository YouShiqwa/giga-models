...
config = dict(
    runners=['pi0.pi05_vggt_trainer.Pi05VGGTTrainer'],
    project_dir='./experiments/vla/pi05/robocasa_set_up_cutting_station_dual_vggt',
    launch=dict(
        gpu_ids=[0, 1, 2, 3],
    ...
    models=dict(
        # Use exactly the same Pi0.5 starting checkpoint as the dual baseline.
        # The loader clones its original Action Expert independently into the
        # mobility and geometry streams; the VGGT reducer is newly initialized.
        ...
        vggt_repo_path=(
            '/vepfs-cnbje63de6fae220/chengy/code/MobileManipLab/policy/starVLA/'
            'playground/third_party/vggt-omega'
        ),
        vggt_checkpoint_path=(
            '/vepfs-cnbje63de6fae220/chengy/code/MobileManipLab/policy/starVLA/'
            'playground/Pretrained_models/VGGT-Omega-1B-256-Text-Alignment/'
        ...
        vggt_num_views=3,
        vggt_output_grid_size=2,
        vggt_norm_groups=32,
        vggt_enable_alignment=True,
        # Model order: [left agentview, wrist, right agentview].
        # VGGT order: [left agentview, right agentview, wrist].
        vggt_view_order=[0, 2, 1],
    ),
    optimizers=dict(
        type='AdamW',
        betas=(0.9, 0.95),
    ...
    train=dict(
        ...
        # Keep the existing local TensorBoard logs and mirror the same metrics
        # to a shared W&B project.  Each experiment uses a distinct run name.
        log_with=['tensorboard', 'wandb'],
        log_interval=1,
        wandb=dict(
            # Set ``entity`` to a W&B team/account name when needed.  Leaving
            # it as None uses the account selected by ``wandb login``.
            entity=None,
            mode='online',
            project='robocasa_pi',
            name='pi05_dual_vggt_from_pi05',
            group='robocasa_set_up_cutting_station',
            job_type='train',
            tags=['robocasa', 'pi05', 'dual-action', 'vggt'],
            config=dict(
                model_variant='pi05_dual_vggt',
